...
def process_metrics(engine, nsd_id, timeout=None):
    Session = get_scoped_session(engine)
    session = Session()
    collectors = session.query(DatasetCollector) \
        .filter(DatasetCollector.nsd_id==nsd_id,
                DatasetCollector.status.in_([CollectorStatus.terminated,
                                             CollectorStatus.processed,
                                             CollectorStatus.processing])).all()
    for c in collectors:
    ...
    try:
        ...
        print("Dataset processing for nsd_id ", nsd_id, "failed:", str(e))
        for c in collectors:
            c.status = CollectorStatus.terminated
    else:
        for c in collectors:
            c.status = CollectorStatus.processed
    finally:
        session.commit()
        Session.remove()
